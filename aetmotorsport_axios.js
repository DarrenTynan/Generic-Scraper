...
const targetURL = "https://www.aetmotorsport.com";
 ...
 */
async function scrapeParentLinks(database)
{
    ...
    .then( (response) =>
    {
        ...
        if(response.status === 200)
        {
            ...
            $('#AccessibleNav > li > a').each((index, element) =>
            {
                ...
                let dbElement = targetURL +  endPoint;
                
                newCollection.url = dbElement;
                
                database.collection('links').insertOne(newCollection);  
            });
...
async function main()
{
    ...
    await scrapeParentLinks(database);
